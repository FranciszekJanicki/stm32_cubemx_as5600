...
as5600_err_t as5600_init_chip(as5600_t* as5600, uint16_t start_raw, uint16_t end_raw)
{
    ...
    as5600_zpos_reg_t zpos = {.zpos = start_raw & 0x0FFF};
    err = as5600_set_zpos_reg(as5600, &zpos);
    if (err != AS5600_ERR_OK)
        return err;

    as5600_mpos_reg_t mpos = {.mpos = end_raw & 0x0FFF};
...
int main(void)
{
    ...
    as5600_initialize(
        &as5600,
        &(as5600_config_t){.dir_pin = GPIO_PIN_2, .min_angle = 0.0F, .max_angle = 360.0F},
        &(as5600_interface_t){
            .gpio_user = GPIOC,
