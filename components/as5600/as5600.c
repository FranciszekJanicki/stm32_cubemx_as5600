...
as5600_err_t as5600_set_direction(as5600_t* as5600, as5600_direction_t direction)
{
    assert(as5600);

    if (as5600->state.direction != direction) {
        as5600->state.direction = direction;
...
as5600_err_t as5600_get_angle_data_raw_adc(as5600_t const* as5600, uint16_t* raw)
{
    ...
    as5600_err_t err = as5600_adc_get_conversion(as5600, &conversion);

    *raw = (as5600->state.direction == AS5600_DIRECTION_CW) ? conversion & 0x0FFFU
                                                            : 4096U - (conversion & 0x0FFFU);
...
as5600_err_t as5600_get_angle_data_raw_bus(as5600_t const* as5600, uint16_t* raw)
{
    ...
    *raw = (as5600->state.direction == AS5600_DIRECTION_CW) ? reg.angle : 4096U - reg.angle;
